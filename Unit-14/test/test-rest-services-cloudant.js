    ...
    testResults.forEach((cleanup) => {
        db.destroy(cleanup.id, cleanup.rev, (err, result) => {
            if (err) {
                ...
                logger.warn('Error deleting fake item record: ' + JSON.stringify(cleanup) + 
                    ', Error message:' + err.message, 'after()');
            } else {
                logger.info('Fake object delete successful: ' + JSON.stringify(result), 'after()');
            }
        });
        utils.eatCpu(500);
    });
        ...
        request('GET', `/rest/items?upc=${ftUpc}`, null, (err, data) => {
            ...
            if (data) {
                try {
                    ...
                    expect(jsonData.docs[0].itemDescription).to.equal(expected_item_description);
...
describe('PUT /rest/lists/', function testListsUpdate() {
    it('should successfully update one and only one row', (done) => {
        request('POST', `/rest/lists`, `{ "description" : "Test Shopping List for testListsUpdate()" }`, (err, data) => {// eslint-disable-line max-len
            ...
            if (data) {
                if (logger.isDebug()) logger.debug('Raw response data: ' + data, 'testListsUpdate()');
                let jsonData = JSON.parse(data);
                // Store the list id and rev so it can be cleaned up
                testResults.push({ id: jsonData.createdId, rev: jsonData.createdRevId });
                let newListId = jsonData.createdId;
...
describe('DELETE /rest/lists/:listId/items/'+ftItemId, function testListsRemoveItem() {
